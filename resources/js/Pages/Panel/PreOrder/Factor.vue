  ...
  <div class="w-full">
    <div class="my-4  p-2 sm:p-4 mx-auto overflow-x-scroll    max-w-3xl" :dir="dir()" v-if="data">
      ...
      <table class="w-full    ">
        ...
        <tbody class="text-sm">
        <tr v-for="(item,idx) in JSON.parse(data.items || '[]')">
          ...
          <td class="border flex flex-col text-center p-2  ">

            <div> {{ item.name_fa }}</div>
            <div class="text-neutral-500 text-xs"> {{ item.name_en }}</div>
          </td>
          <td class="border text-center p-2  ">
            {{
        ...
        <tr>
          ...
          <td colspan="1" class="border text-center p-2">{{
              data.total_shipping_price == null ? __('local') : asPrice(data.total_shipping_price)
            }}
          </td>
        </tr>
