<template>
  <Scaffold navbar-theme="dark">
    ...
    <section>
      <div class="flex   p-4 flex-col bg-primary-50 text-primary-900">
        <div class="border-b-2 p-2 flex ">
          <div>{{ __('agency') }} {{ agency.name }}</div>
        </div>
        <div class="text-sm text-gray-600 p-2 flex  ">
          <div class=" "> {{ getCityName(agency.province_id) }} - {{ getCityName(agency.county_id) }}</div>
          <div class="px-2"> |</div>
...
export default {
  ...
  methods: {
    getData(page) {
      this.log(this.agency);
      this.params.agency_id = this.agency.id;
      if (page == 0) {
        this.params.page = 1;
        this.products = [];
      }
